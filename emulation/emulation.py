...
class Person:
    ...
    def update_balance(self, url, inst_id):
        ...
        price = requests.get(url + 'api/v1/user/price/?instrument_id=' + str(inst_id)).json()['result']
...
def try_instrument(url, bank, inst_id):
    ins = inst_id
    if len(requests.get(url + 'api/v1/user/instrument-balance/?instrument_id=' + str(inst_id),
